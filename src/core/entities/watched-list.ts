export abstract class WatchedList<T> {
  ...
  public update(items: T[]): void {
    /* -> finds items in *items* that are not present in *currentItems* 
       ...
       -> newItems: Contains only the items from items that are not already in currentItems
    */
    ...
    /* -> finds items in *currentItems* that are not present in *items* 
       ...
       -> removedItems: Contains only the items from currentItems that are not already in items
    */
